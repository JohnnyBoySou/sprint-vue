...
function onDragEnd(evt: any) {
  ...
  const newStatus =
    ...
    (evt.to.__vue__?.$attrs['data-status'] as TaskStatus)
  const oldStatus =
    (evt.from.getAttribute('data-status') as TaskStatus) ||
    (evt.from.__vue__?.$attrs['data-status'] as TaskStatus)
  ...
  <div class="kanban-container">
    ...
    <div v-else>
      ...
      <q-banner v-if="unassignedTasks.length" class="bg-grey-2 q-mt-lg" rounded>
        ...
        <div class="text-body2 q-mb-md">Arraste estas tarefas para um board para organizá-las.</div>

        <draggable
          :list="unassignedTasks"
          group="tasks"
          item-key="id"
          @end="onDragEndUnassigned"
          class="q-mt-md"
        >
          <template #item="{ element }">
            <q-card class="q-mb-sm task-card" flat bordered :data-task-id="element.id">
    ...
    <TaskAddModal v-model="showAddTaskModal" @task-added="onTaskAdded" />
  </div>
</template>
